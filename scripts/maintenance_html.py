...
(() => {
  ...
  function render() {
    ...
    el("download-current").disabled=!filtered.some(row=>row.after);
  ...
  try {
    ...
    el("download-current").addEventListener("click",()=>{const records=filtered.map(row=>row.after).filter(Boolean);if(records.length)download({schema_version:1,records},"maintenance-current-records.json");});
...
<section aria-label="Maintenance results"><div class="toolbar"><h2>Maintenance review</h2><div class="actions"><button id="download-report" type="button">Download full review JSON</button><button id="download-current" type="button" class="secondary" disabled>Download matched after/current records</button><button id="download-selection" type="button" class="secondary" disabled>Download filtered review packet</button><button id="expand-visible" type="button" class="secondary" disabled>Expand visible evidence</button><button id="collapse-visible" type="button" class="secondary" disabled>Collapse visible evidence</button><button id="print-page" type="button" class="secondary" disabled>Print this page</button></div></div>
