...
<div id="change-controls"><label for="change-filter">Snapshot change</label><select id="change-filter"><option value="">All changes</option></select></div><div><label for="jurisdiction">Jurisdiction</label><select id="jurisdiction"><option value="">All</option></select></div><div><label for="record-type">Record type</label><select id="record-type"><option value="">All</option></select></div><div><label for="status-filter">Record review status</label><select id="status-filter"><option value="">All</option></select></div><div><label for="reason-filter">Structural triage reason</label><select id="reason-filter"><option value="">All reasons</option></select></div><div id="transition-controls"><label for="transition-filter">Independent field transitions</label><select id="transition-filter"><option value="">All</option><option value="policy_dates_changed">Policy dates changed</option><option value="policy_stage_changed">Policy stage changed</option><option value="verification_status_changed">Review status changed</option></select></div></div></div>
...
<section aria-label="Maintenance results"><div class="toolbar"><h2>Maintenance review</h2><div class="actions"><button id="download-report" type="button">Download full review JSON</button><button id="download-selection" type="button" class="secondary" disabled>Download filtered review packet</button><button id="print-page" type="button" class="secondary" disabled>Print this page</button></div></div>
...
<div class="view-buttons"><button id="view-all" type="button" aria-pressed="true">All snapshot records</button><button id="view-queue" type="button" aria-pressed="false">Review queue only</button></div><p id="counts" role="status" aria-live="polite">Loading local report...</p><div id="error" role="alert" hidden></div><noscript><p class="notice">Enable JavaScript locally to explore this report, or inspect its JSON with the maintenance CLI.</p></noscript>
