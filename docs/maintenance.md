...
Imported maintenance artifacts must be regular JSON files. On POSIX systems, the loader opens a descriptor in nonblocking mode and checks its file type before reading payload bytes, so named pipes do not wait for a writer. Directories and device files are also rejected with a fixed diagnostic. Read-only symbolic links and hard links to regular input files retain their existing behavior; the stricter no-link rules above apply to output destinations. The byte cap is checked before and after reading. These checks are not a wall-clock timeout for arbitrary filesystem I/O.
...
- **Review date:** select known or missing last-review dates without inferring age, correctness or certification.
