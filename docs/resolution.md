...
A filesystem cleanup failure or interruption after publication can leave a complete output even when the command fails. Existing data is never replaced. Inspect the destination and choose a fresh name before retrying. Use real directory paths: a path through a symlink is intentionally rejected. These guarantees require the existing supported POSIX filesystem facilities.
